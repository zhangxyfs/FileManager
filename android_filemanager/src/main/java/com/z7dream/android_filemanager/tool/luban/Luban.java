...
import com.z7dream.android_filemanager_lib.tool.rx.RxSchedulersHelper;
...
public class Luban {
    ...
    private File mFile;
    private static String toZipCompletePath;//首先生成需要压缩成的图片
    private int gear = THIRD_GEAR;
    private String filename;

    private Bitmap.CompressFormat bitFormat = Bitmap.CompressFormat.JPEG;
    ...
    private static File getPhotoCacheDir(Context context, String cacheName) {
        File cacheDir = context.getExternalFilesDir(Environment.DIRECTORY_PICTURES);//context.getCacheDir();
        if (cacheDir != null) {
    ...
    public Luban launch() {
        ...
        if (onLubanListener != null) {
            onLubanListener.onStart();
            onLubanListener.onStart(toZipCompletePath);
        }

        if (gear == Luban.FIRST_GEAR)
            Flowable.just(mFile)
                    .map(this::firstCompress)
                    .compose(RxSchedulersHelper.fio_main())
                    .doOnError(throwable -> {
                        if (compressListener != null) compressListener.onError(throwable);
                        if (onLubanListener != null) onLubanListener.onError(throwable);
                    }).onErrorResumeNext(throwable -> {
                return Flowable.empty();
            }).filter(file -> file != null)
                    .subscribe(file -> {
                        if (compressListener != null) compressListener.onSuccess(file);
                        if (onLubanListener != null) onLubanListener.onSuccess(file);
                    });
        else if (gear == Luban.THIRD_GEAR)
            Flowable.just(mFile).map(this::thirdCompress).compose(RxSchedulersHelper.fio_main())
                    .doOnError(throwable -> {
                        if (compressListener != null) compressListener.onError(throwable);
                        if (onLubanListener != null) onLubanListener.onError(throwable);
                    })
    ...
    public Luban load(File file) {
        mFile = file;
        if (TextUtils.isEmpty(toZipCompletePath))
            toZipCompletePath = mCacheDir.getAbsolutePath() + File.separator +
                    (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + (bitFormat == Bitmap.CompressFormat.PNG ? ".png" : ".jpg");
        return this;
    }
     ...
     * @return
     */
    public Luban setZipCompletePath(String filePath) {
        toZipCompletePath = filePath;
        return this;
    }
    ...
    public Luban setCompressListener(OnLubanListener listener) {
        onLubanListener = listener;
        return this;
    }
    ...
    public Luban setBitFormat(boolean isJEPG) {
        if (isJEPG) {
            bitFormat = Bitmap.CompressFormat.JPEG;
        } else {
            bitFormat = Bitmap.CompressFormat.PNG;
        }
        return this;
    }
    ...
    public Flowable<File> asFlowable() {
        if (gear == FIRST_GEAR)
            return Flowable.just(mFile).map(this::firstCompress);
        else if (gear == THIRD_GEAR)
            return Flowable.just(mFile).map(this::thirdCompress);
        else return Flowable.empty();
    }

    public String getZipCompletePath() {
        return mCacheDir.getAbsolutePath() + File.separator +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + (bitFormat == Bitmap.CompressFormat.PNG ? ".png" : ".jpg");
    }

    private File thirdCompress(@NonNull File file) {
        String thumb = toZipCompletePath;
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String filePath = file.getAbsolutePath();
        String thumbFilePath = toZipCompletePath;
        if (TextUtils.isEmpty(thumbFilePath)) {
            thumbFilePath = mCacheDir.getAbsolutePath() + File.separator +
                    (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + (bitFormat == Bitmap.CompressFormat.PNG ? ".png" : ".jpg");
        }
...
//        return NativeUtil.saveBitmap(thumbFilePath, thbBitmap, size);
    }
    ...
    private File saveImage(String filePath, Bitmap bitmap, long size) {
        ...
        int options = 100;
        int needOptions = (int) (size * options / result.length());
        if (needOptions < 30) needOptions = 30;

        bitmap.compress(bitFormat, needOptions, stream);
